...
\begin{code}
  ...
  dom :: Ty -> Retval Ty
  dom (TyArr d _) = return d
  dom TyBool = Error "Type Error - Cannot find domain of a boolean type"

  ran :: Ty -> Retval Ty
  ran (TyArr _ r) = return r
  ran TyBool = Error "Type Error - Cannot find range of a boolean type"
...
\begin{code}
  ...
  getTypeFromContext c v = if ((length c) >= v) 
			   then return (c!!v)
			   else Error "Type Error - Variable out of scope"
\end{code} 
...
\begin{code}
  ...
  typeof ctx (If t1 t2 t3) =
      if (typeof ctx t1) == return TyBool
         then let tt1 = typeof ctx t2; tt2 = typeof ctx t3 in
                  if tt1 == tt2 then tt2
                     else Error "Type Error - If branches of different types"
         else Error "Type Error - If conditional not boolean"
  ...
  typeof ctx (App t1 t2) = 
	 ...
	 dtt1 <- dom tt1;
         if isArr tt1 then 
            if tt2 == dtt1 then return tt2
               else Error "Type Error - Argument type does not match domain"
            else Error "Type Error - Term is not an abstraction"           
...
\begin{code}
  evalValue :: Term -> Retval Term

  evalValue TmTrue = return TmTrue
  evalValue TmFalse = return TmFalse

  evalValue (App t1 t2) =
      do nt1 <- evalValue t1;  -- call evalValue until nt1 is a lambda
	 nt2 <- evalValue t2;   -- call evalValue until nt2 is a lambda
	 evalValue (shift (subst 0 (shift nt2 0 1) (expr nt1)) 0 (-1))
		   
  evalValue (Lambda ty1 t1) = return (Lambda ty1 t1)
			      
  evalValue (Var t1) = Error "Evaluation Error - Cannot evaluate free variable"

  evalValue (If TmTrue t2 t3) = 
      do tr <- evalValue t2;
	 fl <- evalValue t3;
	 return tr
  evalValue (If TmFalse t2 t3) = 
      do tr <- evalValue t2;
	 fl <- evalValue t3;
	 return fl
  evalValue (If t1 t2 t3) = 
      do cond <- (evalValue t1);
	 evalValue (If cond t2 t3)

  evalLazy :: Term -> Retval Term

  evalLazy TmTrue = return TmTrue
  evalLazy TmFalse = return TmFalse

  evalLazy (App t1 t2) =
      do nt1 <- evalLazy t1;
	 evalLazy (shift (subst 0 (shift t2 0 1) (expr nt1)) 0 (-1))

  evalLazy (Lambda ty1 t1) = return (Lambda ty1 t1)
  
  evalLazy (Var t1) = Error "Evaluation Error - Cannot evaluate free variable"

  evalLazy (If TmTrue t2 _) = evalLazy t2
  evalLazy (If TmFalse _ t3) = evalLazy t3
  evalLazy (If t1 t2 t3) =
      do cond <- (evalLazy t1);
	 evalLazy (If cond t2 t3)
...
--  evalValue _ (App (Lambda ty12 t12) TmTrue) =
--        let nt = shift (subst 0 (shift TmTrue 0 1) t12) 0 (-1) in
--            if (isApp nt) then evalValue [] nt else nt
--  evalValue _ (App (Lambda ty12 t12) TmFalse) =
...
\begin{code}
  evalTemplate :: (Term -> Retval Ty) ->
		  (Term -> Retval Term) ->
		  Term ->
		  Retval Term
  evalTemplate typeof eval term =
...
\begin{code}
  evalV :: Term -> Retval Term
  evalV = evalTemplate (typeof []) evalValue

  evalL :: Term -> Retval Term
